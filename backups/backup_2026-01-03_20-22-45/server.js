...
const CASCO_TARIFFS_DIR = 'D:\\Desktop\\Каско';
const MTPL_TARIFFS_DIR = 'D:\\Desktop\\ГО';
...
app.post('/api/calculate', (req, res) => {
  try {
      ...
      case 'casco':
        ...
        if (insurer) {
          ...
          if (fs.existsSync(insurerFile)) {
            ...
            return res.json({ premium: Math.round(premium * 100) / 100, currency: 'BGN', tariffRate });
          }
        }
    ...
    res.json({ premium: Math.round(premium * 100) / 100, currency: 'BGN' });
  } catch (error) {
        ...
        if (match) {
          ...
          try {
            ...
            if (insuranceType === 'casco') {
              ...
              premium = calculateMTPLPremium(data, tariffData);
            }
...
// Admin API: Get tariffs for specific insurer
app.get('/api/admin/tariffs/:insurer', (req, res) => {
  try {
...
// Admin API: Save tariffs for specific insurer
app.post('/api/admin/tariffs/:insurer', (req, res) => {
  try {
    const insurer = req.params.insurer;
    const data = req.body;
    const adminTariffsDir = path.join(__dirname, 'data', 'admin-tariffs');
  ...
  try {
    ...
    Object.keys(offersByInsurer).forEach(insurerName => {
      ...
      if (insurerData.casco) {
        ...
        const premium = cascoData.premium || 0;
        const tax = premium * 0.02;
        const totalPremium = premium + tax;
        const totalPremiumEUR = (totalPremium / 1.95583).toFixed(2);
          ...
          rows: [
              ...
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({
                    children: [new TextRun({ text: `${parseFloat(insuranceData.insuranceSum || 0).toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв.` })]
                  })],
            ...
            new TableRow({
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({
                    children: [new TextRun({ text: `${premium.toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв.` })]
                  })],
                  width: { size: 40, type: WidthType.PERCENTAGE }
                })
              ]
            ...
            new TableRow({
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({
                    children: [new TextRun({ text: `${tax.toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв.` })]
                  })],
                  width: { size: 40, type: WidthType.PERCENTAGE }
                })
              ]
            ...
            new TableRow({
              children: [
                ...
                new TableCell({
                  children: [new Paragraph({
                    children: [new TextRun({ text: `${totalPremium.toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв. / ${totalPremiumEUR} €`, bold: true })]
                  })],
                  width: { size: 40, type: WidthType.PERCENTAGE }
                })
              ]
        ...
        children.push(
          new Paragraph({
            children: [
              ...
              new TextRun({ text: `еднократно ${totalPremium.toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв. / ${totalPremiumEUR} €` })
            ],
            spacing: { after: 400 }
          })
        );
      ...
      if (insurerData.mtpl) {
        ...
        const premium = mtplData.premium || 0;
        const tax = premium * 0.02;
        const gfOf = 16.00;
        const totalPremium = premium + tax + gfOf;
        const totalPremiumEUR = (totalPremium / 1.95583).toFixed(2);
        ...
        const mtplTable = new Table({
          rows: [
            ...
            new TableRow({
              children: [
                new TableCell({
                  children: [new Paragraph({ children: [new TextRun({ text: 'Вноска за ГФ И ОФ съгласно КЗ:', bold: true })] })],
                  width: { size: 60, type: WidthType.PERCENTAGE }
                }),
                new TableCell({
                  children: [new Paragraph({
                    children: [new TextRun({ text: `${gfOf.toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} лв.` })]
                  })],
                  width: { size: 40, type: WidthType.PERCENTAGE }
                })
              ]
      ...
      if (insurerData.gap) {
        ...
        const premium = gapData.premium || 0;
        const tax = premium * 0.02;
        const totalPremium = premium + tax;
        const totalPremiumEUR = (totalPremium / 1.95583).toFixed(2);
