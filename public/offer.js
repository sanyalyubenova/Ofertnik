...
function renderMTPLRow(offers, insuranceType) {
    ...
    offers.forEach(offer => {
        ...
        html += `<td class="premium">${mtplData?.basePremium ? mtplData.basePremium.toFixed(2) + ' €' : '-'}</td>`;
    });
    html += '</tr>';
    ...
    offers.forEach(offer => {
        const mtplData = offer.insuranceTypes?.mtpl;
        html += `<td class="premium">${mtplData?.gf ? mtplData.gf.toFixed(2) + ' €' : '-'}</td>`;
    });
    html += '</tr>';
    ...
    offers.forEach(offer => {
        const mtplData = offer.insuranceTypes?.mtpl;
        html += `<td class="premium">${mtplData?.of ? mtplData.of.toFixed(2) + ' €' : '-'}</td>`;
    });
    html += '</tr>';
    ...
    offers.forEach(offer => {
        const mtplData = offer.insuranceTypes?.mtpl;
        html += `<td class="premium">${mtplData?.premium ? mtplData.premium.toFixed(2) + ' €' : '-'}</td>`;
    });
    html += '</tr>';
...
async function recalculateCascoPremium(insurerIndex) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const premiumCell = document.querySelector(`.casco-premium[data-insurer-index="${insurerIndex}"]`);
            if (premiumCell) {
                premiumCell.textContent = result.premium.toFixed(2) + ' €';
            }
